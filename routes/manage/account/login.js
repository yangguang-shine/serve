...
module.exports = async function login() {
    ...
    if (phoneInfoList.length) {
        ...
        if (encryptPassword === phoneInfo.encryptPassword) {
            ...
            const res = await this.querySQL('select manageToken from manage_token_store where manageID = ?', [manageID])
            if (res.length) {
                if (res.length > 1) {
                    console.log('多个manageID')
                }
                await this.querySQL(`update manage_token_store set manageToken = ? where manageID = ?`, [manageToken, manageID])
            } else {
