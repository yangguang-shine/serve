    ...
    try {
        ...
        await ctx.SQLtransaction(async (querySQL) => {
            ...
            if (queryUserIDList.length === 1) {
                sql = `update user_info_pass set session_key = ? where openid = ?`
                await querySQL(sql, [session_key, openid])
                userID = queryUserIDList[0].userID
            } else if (!queryUserIDList.length) {
                sql = `insert into user_info_pass (session_key, openid) values (?, ?)`
            ...
            sql = `select token from user_token_store where userID = ?`
            const findToken = await querySQL(sql, [userID])
            // 增加更新用户token
            if (findToken.length === 1) {
                sql = `update user_token_store set token = ?, secret = ?  where userID = ?`
                await querySQL(sql, [token, secret, userID])
            } else if (!findToken.length) {
                sql = `insert into user_token_store (userID, token, secret) values (?, ?, ?)`
